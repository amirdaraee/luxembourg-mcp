...
luxembourg-data search_statistics '{"query":"population","limit":5}' --output-limit 5
...
returns source URLs from the provider. By default, top-level result lists are
limited to 20 items; use `--output-limit` (`--limit` is an alias),
`--summary-only`, `--full`, or `--save PATH` to control output size.
